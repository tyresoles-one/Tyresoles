/**
 * Accounts report REST API client.
 * Report meta: Tyresoles.Web ReportsController.GetAccountsReportMeta (GET api/reports/accounts/meta).
 * Report generation: POST api/reports/accounts.
 */
...
export type AccountsReportSlug = string;
...
/**
 * Fetch accounts report metadata (ReportsController.GetAccountsReportMeta).
 */
export async function getAccountsReportMeta(userId?: string | null): Promise<ReportMeta[]> {
  const base = getBackendBaseUrl();
  const reportsBase = `${base}/api/reports`;
  const url = new URL(`${reportsBase}/accounts/meta`);
...
/**
 * Export accounts report as file (PDF, Excel, Word).
 */
export async function exportAccountsReport(
  slug: AccountsReportSlug,
  params: ReportFetchParams,
  format?: string,
): Promise<{ blob: Blob; fileName: string }> {
  const base = getBackendBaseUrl();
  const reportsBase = `${base}/api/reports`;
  const url = `${reportsBase}/accounts`;
