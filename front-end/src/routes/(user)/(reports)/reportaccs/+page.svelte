<script lang="ts">
  ...
  import {
    ...
    getAccountsReportMeta,
  } from "$lib/services/accountsReports";
  ...
  /** Determine if user has multiple locations, and if we should show respCenters */
  const userLocations = $derived($authStore.locations ?? []);
  const showRespCentersSelect = $derived(userLocations.length > 1);
  ...
  const initialValues: ReportAccountsForm = {
    report: "",
    dateRange: undefined,
    view: "All",
    type: "",
  ...
  const form = new VenForm<ReportAccountsForm>({ initialValues });

  let reportMetaList = $state<Awaited<
    ReturnType<typeof getAccountsReportMeta>
  > | null>(null);
  ...
  const enrichMeta = (list: any[]) => {
    const metaMap: Record<string, any> = {
      "GST Report 01": { showType: true, typeOptions: ["Invoice", "Credit Memo"], showRespCenters: true, requiredFields: "dateRange, type", datePreset: "thisMonth,lastMonth", outputFormats: "pdf,excel" },
      "GST Report 02": { showType: true, typeOptions: ["Invoice", "Credit Memo"], showRespCenters: true, requiredFields: "dateRange, type", datePreset: "thisMonth,lastMonth", outputFormats: "pdf,excel" },
      "GST Report 03": { showType: true, typeOptions: ["Invoice", "Credit Memo"], showRespCenters: true, requiredFields: "dateRange, type", datePreset: "thisMonth,lastMonth", outputFormats: "pdf,excel" },
      "GST Report 04": { showType: true, typeOptions: ["Invoice", "Credit Memo"], showRespCenters: true, requiredFields: "dateRange, type", datePreset: "thisMonth,lastMonth", outputFormats: "pdf,excel" },
      "GST Report 05": { showType: true, typeOptions: ["Invoice", "Credit Memo"], showRespCenters: true, requiredFields: "dateRange, type", datePreset: "thisMonth,lastMonth", outputFormats: "pdf,excel" },
      "GST Report 06": { showType: true, typeOptions: ["Invoice", "Credit Memo"], showRespCenters: true, requiredFields: "dateRange, type", datePreset: "thisMonth,lastMonth", outputFormats: "pdf,excel" },
      "E Invoice Errors": { requiredFields: "dateRange", datePreset: "thisMonth,lastMonth", outputFormats: "pdf,excel" },
    };
  ...
  onMount(() => {
    ...
    getAccountsReportMeta(user?.userId?.toString() ?? undefined)
      .then((rawList) => {
        const list = enrichMeta(rawList);
        console.log("Accounts meta", list);
        reportMetaList = list;
  ...
  const schema = $derived.by<FormSchema>(() => {
    ...
    if (selectedMeta?.showRespCenters && showRespCentersSelect) {
      fields.push({
        type: "custom",
  ...
  $effect(() => {
    ...
    if (requiredKeys.includes("dateRange"))
      zodShape.dateRange = z
        ...
        .refine((d) => d && d.start && d.end, "Date range is required");
    ...
    if (requiredKeys.includes("respCenters") && showRespCentersSelect)
      zodShape.respCenters = z.string().min(1, "Resp Center is mandatory");
    if (requiredKeys.includes("customers"))
      zodShape.customers = z.string().min(1, "Please select at least one customer");
    if (requiredKeys.includes("nos"))
      zodShape.nos = z.string().min(1, "Please enter at least one Document No");

    form.setSchema(z.object(zodShape) as unknown as ZodSchema<ReportAccountsForm>);
  });
  ...
  function getParams(
    ...
    // Evaluate respCenters
    const useRespCentersSelect = (authStore.get().locations ?? []).length > 1;
  ...
  async function generateReport(format: string) {
    ...
    try {
      ...
      const { blob, fileName } = await exportAccountsReport(form.values.report, params);
