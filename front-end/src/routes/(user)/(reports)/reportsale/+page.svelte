<script lang="ts">
  ...
  /** Reports filter from stored auth: menu item with action "/reportsale" options (comma-separated report codes). */
  function getReportsaleOptions(): string | undefined {
    const menus = authStore.get().menus ?? [];
    for (const menu of menus) {
      for (const sub of menu.subMenus ?? []) {
        for (const item of sub.items ?? []) {
          const action = (item.action ?? "").trim().replace(/\/+$/, "") || "";
          if (action === "/reportsale")
  ...
  /** Locations where sale === 1 (user can run sales reports for these). */
  const saleLocations = $derived.by(() => {
    const locs = $authStore.locations ?? [];
    return locs.filter((l: { sale?: unknown }) => l.sale === 1);
  });
  ...
  const initialValues: ReportSaleForm = {
    report: "",
  ...
  const form = new VenForm<ReportSaleForm>({ initialValues });

  let reportMetaList = $state<Awaited<
    ReturnType<typeof getSalesReportMeta>
  > | null>(null);
  ...
  $effect(() => {
    ...
    form.setSchema(z.object(zodShape) as unknown as ZodSchema<ReportSaleForm>);
  });
  ...
  function getParams(
    ...
    const user = getUser();
    // Use form respCenters only when user has >1 sale location (select is shown); else use user.respCenter
    const saleLocs = (authStore.get().locations ?? []).filter(
      (l: { sale?: unknown }) => l.sale === 1,
    );
    const useRespCentersSelect = saleLocs.length > 1;
